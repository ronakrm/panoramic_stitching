...
for i=1:numwindows
    ...
    tic;
        ...
        for j = 1:size(theMap.keys,2)
           ...
           meanr = uint8(ceil(mean(cell2mat(r))));
           meang = uint8(ceil(mean(cell2mat(g))));
           meanb = uint8(ceil(mean(cell2mat(b))));
           
           y = ceil(double(key)/double(destwidth));
           x = key -  (y-1)*destwidth;
           
           output(y,x,:) = [meanr;meang;meanb];
       end
    toc;
end
